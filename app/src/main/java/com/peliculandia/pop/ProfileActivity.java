...
import com.peliculandia.pop.network.apis.DeactivateAccountApi;
import com.peliculandia.pop.network.apis.ProfileApi;
import com.peliculandia.pop.network.apis.SetPasswordApi;
import com.peliculandia.pop.network.apis.UserDataApi;
import com.peliculandia.pop.network.model.ResponseStatus;
import com.peliculandia.pop.network.model.User;
import com.peliculandia.pop.utils.Constants;
import com.peliculandia.pop.utils.FileUtil;
...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String txttituloperfil = getString(R.string.txttituloperfil);

        Toast.makeText(this, "" + txttituloperfil, Toast.LENGTH_SHORT).show();

        setSupportActionBar(toolbar);
        getSupportActionBar().setTitle(txttituloperfil);
        ...
        idUsuarioBase = PreferenceUtils.getUserId(ProfileActivity.this);

        btnUpdate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (etEmail.getText().toString().equals("")) {
                    Toast.makeText(ProfileActivity.this, "Porfavor ingrese email.", Toast.LENGTH_LONG).show();
                    return;
                } else if (etName.getText().toString().equals("")) {
                    Toast.makeText(ProfileActivity.this, "Porfavor ingrese nombre.", Toast.LENGTH_LONG).show();
                    return;
                } else if (etCurrentPassword.getText().toString().equals("")) {
                    new ToastMsg(ProfileActivity.this).toastIconError("La contraseña actual no debe estar vacía.");
                    return;
                }
                ...
                updateProfile(idUsuarioBase, email, phone, name, pass, currentPass);
    ...
    private void showDeactivationDialog() {
        ...
        okBt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String txtintroduzca = getString(R.string.ingresecontraseña);
                String txtintroduzca2 = getString(R.string.ingresepeticion);

                if (TextUtils.isEmpty(pass)) {
                    new ToastMsg(ProfileActivity.this).toastIconError(txtintroduzca);
                    return;
                } else if (TextUtils.isEmpty(reason)) {
                    new ToastMsg(ProfileActivity.this).toastIconError(txtintroduzca2);
                    return;
                }
    ...
    private void deactivateAccount(String pass, String reason, final AlertDialog alertDialog, final ProgressBar progressBar) {
        ...
        String txtsaliomal = getString(R.string.algosaliomal);
        DeactivateAccountApi api = retrofit.create(DeactivateAccountApi.class);
        Call<ResponseStatus> call = api.deactivateAccount(idUsuarioBase, pass, reason, AppConfig.API_KEY);
        call.enqueue(new Callback<ResponseStatus>() {
            ...
            public void onResponse(Call<ResponseStatus> call, retrofit2.Response<ResponseStatus> response) {
                if (response.code() == 200) {
                    ...
                    new ToastMsg(ProfileActivity.this).toastIconError(txtsaliomal);
                    alertDialog.dismiss();
                }
            }
    ...
    private void updateProfile(String idString, String emailString, String phoneString, String nameString, String passString, String currentPassString) {
        ...
        call.enqueue(new Callback<ResponseStatus>() {
            ...
            @Override
            public void onFailure(Call<ResponseStatus> call, Throwable t) {
                new ToastMsg(ProfileActivity.this).toastIconSuccess(getString(R.string.cambiosRealizados));
                progressBar.setVisibility(View.GONE);
                Log.e(TAG, t.getLocalizedMessage());
                Log.d("focus2","entre a error 4");
                Toast.makeText(ProfileActivity.this, getString(R.string.abrircerrar), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void showSetPasswordDialog() {
        String contraseñano = getString(R.string.passwordnocoincide);
            ...
            public void onClick(View view) {
                ...
                if (!password.isEmpty() && !confirmPass.isEmpty()) {
                    if (password.equals(confirmPass)) {
                        ...
                        confirmPassEt.setError(contraseñano);
                        new ToastMsg(view.getContext()).toastIconError(contraseñano);
                    }
                }
            }
        });
    ...
    private void setPassword(String password) {
        ...
        Call<ResponseStatus> call = api.setPassword(AppConfig.API_KEY, idUsuarioBase, password, uid);
        call.enqueue(new Callback<ResponseStatus>() {
            @Override
            public void onResponse(Call<ResponseStatus> call, Response<ResponseStatus> response) {
                if (response.code() == 200) {
                    if (response.body() != null) {
                        String okcambiadapass = getString(R.string.contraseñaokcambiada);
                        ResponseStatus status = response.body();
                        if (status.getStatus().equalsIgnoreCase("success")) {
                            new ToastMsg(ProfileActivity.this).toastIconSuccess(okcambiadapass);
